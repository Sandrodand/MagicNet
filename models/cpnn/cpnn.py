...
class cPNN:
    ...
    def learn_one(
        ...
        if len(self.predictions) > 0 and timestamp in self.predictions:
            self.columns_perf[-1].update(y, self.predictions[timestamp])
    ...
    def predict_one(
        ...
        if x is None:
            self.predictions[timestamp] = None
            return None
        with torch.no_grad():
            if not self.loss_on_seq:
            ...
            self.predictions[timestamp] = pred
        return pred
