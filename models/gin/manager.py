...
class CPGmanager(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if model_class==PiggyBackGRU:
            self.model = PiggyBackGRU(input_size=self.input_size, device=self.device, hidden_size=hidden_size,
                                      mask_init=mask_init, threshold_fn=threshold_fn, seq_len=seq_len,
                                      cGRU_weights=cGRU_weights, cap_sigmoid=self.cap_sigmoid)
            self.model.train()
    ...
    def add_new_column(self, task_id):
        ...
        self.ensemble["Expand"] = PiggyBackGRU(input_size=self.input_size, device=self.device,
                                               hidden_size=self.model.hidden_size, mask_init=self.mask_init,
                                               threshold_fn=self.threshold_fn, seq_len=self.seq_len, cap_sigmoid=self.cap_sigmoid)
        ...
        for i in reversed(masklist):
            for name,mask in summed.items():
                if len(i[name].size()) == 2:
                    mask[:i[name].size()[0],:i[name].size()[1]] = mask[:i[name].size()[0],:i[name].size()[1]] + i[name]
                    denom[name][:i[name].size()[0],:i[name].size()[1]] = denom[name][:i[name].size()[0],:i[name].size()[1]] + torch.ones_like(i[name])
                else:
                    mask[:i[name].size()[0]] = mask[:i[name].size()[0]] + i[name]
                    denom[name][:i[name].size()[0]] = denom[name][:i[name].size()[0]] + torch.ones_like(i[name])
        for name,mask in summed.items():
        ...
        if self.in_expansion:
            ...
            if self.ensemble_counter == self.ensemble_batches:
                ...
                if self.ensemble_mode == "classic" or self.curr_task_idx <= 2:
                    if self.current_perf["Expand"].get() > self.ensemble_th*self.current_perf["PiggyMask"].get():
                        best_model = "Expand"
                    else:
                        best_model = "PiggyMask"
                else:
                    if self.current_perf["PiggyMask"].get() > self.current_perf["PiggyMask_last"].get():
                        bestpiggy = "PiggyMask"
                    else:
                        bestpiggy = "PiggyMask_last"
                    if self.current_perf["Expand"].get() > self.current_perf["Expand_last"].get():
                        bestexpand = "Expand"
                    else:
